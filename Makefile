# This Makefile is for the DBD::Multiplex extension to perl.
#
...
#     INC => undef
#     NAME => q[DBD::Multiplex]
#     VERSION_FROM => q[Multiplex.pm]
#     dist => { COMPRESS=>q[gzip], SUFFIX=>q[.gz] }
#     realclean => { FILES=>q[*.xsi] }
...
TO_INST_PM = Multiplex.pm \
	example.pl

PM_TO_BLIB = example.pl \
	$(INST_LIB)/DBD/example.pl \
	Multiplex.pm \
...
DISTNAME = DBD-Multiplex
DISTVNAME = DBD-Multiplex-2.04
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  Multiplex.pm $(INST_MAN3DIR)/DBD::Multiplex.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(INST_LIB)/DBD/example.pl *.xsi $(INST_LIB)/DBD/Multiplex.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         DBD-Multiplex' >> META.yml
	$(NOECHO) $(ECHO) 'version:      2.04' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: Multiplex.pm' >> META.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="2,04,0,0">' > $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  example.pl $(INST_LIB)/DBD/example.pl \
	  Multiplex.pm $(INST_LIB)/DBD/Multiplex.pm 
	$(NOECHO) $(TOUCH) $@
...
DBI_INSTARCH_DIR=/Library/Perl/5.8.6/darwin-thread-multi-2level/auto/DBI
DBI_DRIVER_XST=/Library/Perl/5.8.6/darwin-thread-multi-2level/auto/DBI/Driver.xst
...
$(BASEEXT).xsi: $(DBI_DRIVER_XST) /Library/Perl/5.8.6/darwin-thread-multi-2level/auto/DBI/Driver_xst.h
